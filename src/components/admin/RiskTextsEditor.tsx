...
export default function RiskTextsEditor({ experience, riskId, texts, edited }: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <EditorForm key={resets} experience={experience} riskId={riskId} texts={texts} />
      {edited && (
...
function EditorForm({ experience, riskId, texts }: Omit<Props, "edited">) {
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            >
              <input
                ...
                name="correct"
                value={i}
                defaultChecked={texts.correct === i}
                required
                aria-label={`La opción ${OPTION_KEYS[i]} es la correcta`}
              />
              {OPTION_KEYS[i]}
            </label>
            <input

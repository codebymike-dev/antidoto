...
/** Partidas jugadas con este juego, con enlace a su reporte. */
export default function MatchHistory({ gameId, matches }: { gameId: number; matches: MatchSummary[] }) {
  return (
    <section style={{ ...card, display: "flex", flexDirection: "column", gap: 12, maxWidth: 860 }}>
      <h2 style={{ ...calSans, fontSize: 18, margin: 0, color: colors.ink, fontWeight: 400 }}>Partidas jugadas</h2>
      {matches.length === 0 ? (
        <p style={{ margin: 0, color: colors.muted, fontSize: 13.5 }}>Todavía no se jugó ninguna partida con este juego.</p>
      ) : (
            ...
            <li key={m.id} style={{ borderTop: i ? `1px solid ${colors.accentTint}` : "none" }}>
              ...
              >
                ...
                <span style={{ fontSize: 13, color: colors.muted, flex: 1 }}>
                  {m.players === 1 ? "1 jugador" : `${m.players} jugadores`} · {matchStatusLabel(m.status, m.started_at)}
                  {m.host ? ` · host ${m.host}` : ""}
                </span>

...
describe("serie", () => {
  test("la ruta del café va de la finca a la tienda, en orden", () => {
    assert.deepEqual(
      seriesFrom(RUTA_CAFE_FINCA).map((d) => d.key),
      [RUTA_CAFE_FINCA.key, RUTA_CAFE_TRANSPORTE.key, RUTA_CAFE_TRILLADORA.key, RUTA_CAFE_TOSTION.key, RUTA_CAFE_TIENDA.key],
    );
    assert.deepEqual(
      seriesFrom(RUTA_CAFE_TOSTION).map((d) => d.key),
      [RUTA_CAFE_TOSTION.key, RUTA_CAFE_TIENDA.key],
    );
  });
  ...
  test("el puntaje de la serie cuenta los riesgos de todas las estaciones", () => {
    ...
    // 7 de 35 riesgos resueltos, todos a la primera.
    assert.equal(total, 35);
    assert.equal(s.avance, 20);
    assert.equal(s.puntaje, 2);
  });
});
